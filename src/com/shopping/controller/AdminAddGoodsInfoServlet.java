...
/**
 * 这是添加一条商品信息的servlet，在里面逐条拿到所有信息，然后该写入数据库的写入数据库，
 * 该上传文件的上传文件，之中判断了一下，如果文件存在的话，返回一个msg到前台，
 * 如果上传成功也返回一个msg
 */
@WebServlet("/admin/adminAddGoodsInfo.do")
public class AdminAddGoodsInfoServlet extends HttpServlet {
		...
		//	System.out.println(">>>>"+goods_curprice);
			...
			for (int i = 0; i < 5; i++) {
				if(goods_img[i] != "" && !goods_img[i].isEmpty()) {
					...
					goods_img[i] = "goods.jpg";
					System.out.println(goods_img[i]);
				}
				sb_goods_img.append(goods_img[i] + ",");
			}
			gi.setGoods_img(sb_goods_img.toString());
			gi.setGoods_createtime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
			gid.add(gi);
			if(flag_file_exists) {
				updateGoodsDetailWrongMsg = "商品信息添加成功，但是上传的图片已经存在，未进行覆盖，请更改文件名以后再试!";
				...
				updateGoodsDetailWrongMsg = "商品信息添加成功!";
				request.setAttribute("updateGoodsDetailWrongMsg", updateGoodsDetailWrongMsg);
				request.getRequestDispatcher("/admin/goods.jsp").forward(request, response);
			}
		} catch (Exception e) {
